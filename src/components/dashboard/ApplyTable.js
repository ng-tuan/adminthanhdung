...
const ApplyTables = () => {
  ...
  const init = useCallback(() => {
    setLoading(true);
    Q.when(
      fetch("https://demo.annk.info/api/applicants/applicant-status/submit")
    )
      .then(async (res) => {
        if (res.ok) {
                ...
                {map(data, (row) => (
                  <>
                    <tr key={row.applicant._id}>
                      <td>
                        <Link
                          to={`/apply/${row.applicant._id}?status=submit`} // Replace "#" with your actual link or use a proper route
                          style={{
                            cursor: "pointer",
                            textDecoration: "none",
                          }}
